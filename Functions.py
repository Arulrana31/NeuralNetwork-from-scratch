    ...
    def cost_normal(self, value, wanted_value):
        if self.name == "MSE":
            if value.shape != (1, 1) or wanted_value.shape != (1, 1):
                print("MSE can only take single values")
            else:
                return (value[0][0] - wanted_value[0][0]) ** 2

        elif self.name == "MAE":
            if value.shape != (1, 1) or wanted_value.shape != (1, 1):
                print("MAE can only take single values")
            else:
            ...
            if value.shape != wanted_value.shape:
                print(
                    "CrossEntropy requires value and wanted_value to have the same shape"
                )
            else:
    ...
    def cost_der(self, value, wanted_value):
        if self.name == "MSE":
            ...
            if value.shape != k.shape:
                print("Please subtract column arrays in cross entropy derivative")
            else:
